...
def play(black_strategy, white_strategy):
    ...
    board = initial_board
    player = BLACK # Black goes first in Othello
    ...
    while player is not None:
        ...
        if not any_legal_move(player, board) and not any_legal_move(opponent(player), board):
            ...
            print('BLACK: ' + score(BLACK, board))
            print('WHITE: ' + score(WHITE, board))

def next_player(board, prev_player):
    # which player should move next?  Returns None if no legal moves exist
    return opponent(prev_player) if any_legal_move(opponent(prev_player), board) is not None else None
...
def minimax_strategy(node, depth, player, board):
    if depth == 0 or not legal_moves(player, board):
        return score(player, board)

    if player == MAX:
        value = float('-inf')
        new_board = make_move(node, player, deepcopy(board))
        for child in legal_moves(player, new_board):
            value = max(value, minimax_strategy(child, MAX_DEPTH-1, opponent(player)))
        ...
        new_board = make_move(node, player, deepcopy(board))
        for child in legal_moves(player, new_board):
            value = min(value, minimax_strategy(child, MAX_DEPTH-1, opponent(player)))
...
def minimax(player, board):
    ...
    for move in legal_moves(player, board): ####
        minimax_move = minimax_strategy(move, MAX_DEPTH - 1, opponent(player), player, deepcopy(board))

        if max_score < minimax_move:
            max_score = minimax_move
            max_move = move
